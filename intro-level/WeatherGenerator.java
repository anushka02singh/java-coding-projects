...
public class WeatherGenerator {
    ...
    public static int[] forecastGenerator( double drywetProbability, double wetwetProbability, int numberOfDays) 
    {
        ...
        {
            randomNum = StdRandom.uniform();
            if (output[i - 1] == 1) 
            {
                if (randomNum < wetwetProbability) 
                { 
                    output[i] = 1;
                } 
                else 
            ...
            {
                if (randomNum < drywetProbability) 
                {
                    output[i] = 1;
                } 
                else 
    ...
    public static int[] oneMonthForecast(int numberOfLocations, int month, double longitude, double latitude )
    {
        ...
        double drywetProbability2 = drywetProbability[month];
        double wetwetProbability2 = wetwetProbability[month];
        
        return forecastGenerator(drywetProbability2, wetwetProbability2, numberOfDaysInMonth[month]);
    }
