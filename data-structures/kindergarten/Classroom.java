...
public class Classroom {
    ...
    public void makeClassroom ( String filename ) 
    {
        ...
        for(int x = 0; x < totalStudents; x++)
        {
            ...
            Student kid = new Student(firstName, lastName, height);
            SNode temp = new SNode(kid, null);
            ...
            else 
            {
                while(ptr != null)
                {
                    if(temp.getStudent().compareNameTo(ptr.getStudent()) <= 0)
                    {
                        temp.setNext(ptr);
                        if(prev == null)
                        {
    ...
    public void seatStudents () 
    {
	    ...
	    if(musicalChairs != null)
            {
                child = musicalChairs.getStudent();
                SNode all = new SNode(child,studentsInLine);
                studentsInLine = all;
                musicalChairs = null;
            }
        ...
        for(int i = 0; i < studentsSitting.length; i++)
        {
            for(int j = 0; j < studentsSitting[0].length; j++)
            {
                if(seatingAvailability[i][j] == false && studentsSitting[i][j] != null)
                {
                    ...
                    SNode tempN = new SNode(temp, null);
    ...
    public void playMusicalChairs()
    {
        ...
        while (ptr != null)
        {
            ptr = ptr.getNext();
            count++;
            if (ptr.equals(front)) break;
        }

        while (count > 1)
        {
           int others = StdRandom.uniform(count);
           ptr = musicalChairs.getNext();
           SNode prev = musicalChairs;
           SNode remaining = null;

           if (others == count - 1)
           {
            remaining = musicalChairs;
            musicalChairs = musicalChairs.getNext();
            
            while (!remaining.equals(musicalChairs.getNext()))
            {
                musicalChairs = musicalChairs.getNext();
            }

            musicalChairs.setNext(remaining.getNext());
            count--;
           }
           else
           {
           ...
           if (ptr == null){
            studentsInLine = remaining;
        }
        else
        {
            while (ptr != null)
            {
                if (remaining.getStudent().getHeight() <= ptr.getStudent().getHeight())
                {
                    remaining.setNext(ptr);
                    if (prev == null){
                        studentsInLine = remaining;
                    }else{
    ...
    {
        Student temp = new Student(firstName, lastName, height);
        SNode tempN = new SNode(temp, null);
        ...
        {
            SNode ptr = musicalChairs;
            tempN.setNext(musicalChairs.getNext());
            ptr.setNext(tempN);
            musicalChairs = tempN;
        }
        else
