...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        Graph mind = new Graph();
        
        String neuronName = " ";
        for(int i = 0; i < numOfNeurons; i++)
        {
            neuronName = StdIn.readString();
            Vertex temp = new Vertex(neuronName);
            mind.addNeurons(temp);
        }
        ...
        for(int i = 0; i < numOfSynapses; i++)
        {
            ...
            mind.getNeuron(temp).addToSynapses(StdIn.readString());
        }
        ...
        for(int i = 0; i < numOfNeurons; i++)
        {
            Vertex temp = (Vertex)mind.getNeuronList().get(i);
            if(temp.getSynapsesSize() == 0)
            {
                mindStone = temp.getNeuronname();
            }
        }
        ...
        {
            Vertex temp = (Vertex)mind.getNeuronList().get(i);
            if(temp.getSynapsesList().contains(mindStone))
            {
                StdOut.println(temp.getNeuronname());
            }
        }
