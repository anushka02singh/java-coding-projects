...
public class PredictThanosSnap 
{
    ...
    public static void main (String[] args) 
    {
        ...
        Graph people = new Graph(numOfPeople);
        ...
        for(int i = 0; i < numOfPeople; i++) //read adj matrix and read 1 = edge or 0 = no edge
        {
            for(int j = 0; j < numOfPeople; j++)
            {
                if(StdIn.readInt() == 1)
                {
                    people.getVertex(i).addToPeopleEdge(j);
        ...
        int count = 0;
        Vertex ptr = null;
        //System.out.println(people.getVertex(0).toString());
        
        for(int i = 0; i < numOfPeople; i++)
        {
            if(people.getVertex(i).getEdgesSize() != 0)
            {
                stack.add(people.getVertex(i));
                //System.out.println(people.getVertex(i).getPeopleNode());
                break;
            }
        }
        ...
        do
        {
            ...
            {
                if(people.getVertex(i).getVisted() == false)
                {
                    stack.add(people.getVertex(i));
                }
            }
